<!-- Make sure you put this AFTER Leaflet's CSS -->
   ...
   <script type="text/javascript">
	 ...
	 <?php 
   	...
   	let latLng=[-8.145076, 112.647458];
	...
	<?php
		...
		foreach ($getKecamatan->result() as $row) {
			...
			layer: new L.GeoJSON.AJAX(["assets/filegs/'.$row->geojson.'"],{onEachFeature:popUp,style: myStyle'.$row->id_wilayah.',pointToLayer: featureToMarker }).addTo(map)
			}';
		}
	?>
	...
	// for(i=0;i<hotspotPoint.length;i++){
	// 	var data=hotspotPoint[i].properties;
	// 	if(data.lat !=null && data.lng !=null){
	// 		var marker  = L.marker([data.lat,data.lng])
	// 				.bindPopup(
	// 						"Nama : "+data.nama+"<br>"+
	// 						"Alamat : "+data.alamat+"<br>"+
	// 						"Kecamatan : "+data.lokasi+"<br>"+
